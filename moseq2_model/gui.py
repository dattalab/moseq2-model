...
from moseq2_model.command_helpers.data import process_indexfile, select_data_to_model, \
...
def learn_model_command(input_file, dest_file, config_file, index, hold_out, nfolds, num_iter,
    ...
    index_data, all_keys, groups = process_indexfile(index, config_data, data_dict, data_metadata)

    use_keys, use_groups = select_data_to_model(index_data)
    ...
    model_parameters, train_list, hold_out_list = prepare_model_metadata(data_dict, data_metadata, config_data,
                           alpha, gamma, kappa, nkeys, all_keys, hold_out, nfolds, robust, max_states, separate_trans)
    ...
    img_path = graph_modeling_loglikelihoods(iter_lls, iter_holls, group_idx, hold_out, verbose, percent_split, nfolds,
                                  separate_trans, dest_file)
